...
// Primary links shown directly in the desktop nav
const primaryLinks = [
  { name: "Home", to: "/", icon: FaHome },
  { name: "About", to: "/about", icon: FaInfoCircle },
  { name: "Plantation Drives", to: "/plantation-drives", icon: FaTree },
...
// Secondary/less-used links grouped under "More"
const moreLinks = [
  { name: "Causes", to: "/causes", icon: FaTree },
  { name: "Certifications", to: "/certifications", icon: FaUsers },
  { name: "Internship", to: "/internship-form", icon: FaClipboardList },
];

// Full flat list (used for mobile drawer + bottom nav, same order as before)
const navLinks = [
  { name: "Home", to: "/", icon: FaHome },
  { name: "About", to: "/about", icon: FaInfoCircle },
  { name: "Plantation Drives", to: "/plantation-drives", icon: FaTree },
    ...
    <>
      ...
      >
        ...
        >
          ...
          {/* Desktop Menu */}
          <div className="hidden lg:flex items-center gap-6 font-medium text-text-secondary">
            ...
            <div className="relative" ref={moreRef}>
              <button
                ...
                className={
                  (isMoreActive ? activeLink : navLink) +
                  " transition-all duration-200 flex items-center gap-1 focus:outline-none"
                }
                aria-haspopup="true"
              ...
              {moreOpen && (
                <div
                  className="absolute top-full right-0 mt-3 w-56 bg-surface rounded-xl shadow-theme border border-bg py-2 z-50"
                >
                  {moreLinks.map(({ to, name, icon: Icon }) => (
                    <Link
                      key={to}
                      to={to}
      ...
      {/* SIDE DRAWER MENU (Mobile) — unchanged, keeps the full flat list */}
      <div
        className={`fixed top-0 left-0 h-full w-fit bg-surface shadow-theme z-50 transform transition-transform duration-300 ${
          open ? "translate-x-0" : "-translate-x-full"
        }`}
